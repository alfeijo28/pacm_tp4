...
//*INICIALIZANDO AS VARIÁVEIS DO ALLEGRO*//
...
ALLEGRO_BITMAP  *start   = NULL; // variável que vai receber uma imagem
ALLEGRO_BITMAP  *map   = NULL; // variável que vai receber uma imagem
ALLEGRO_BITMAP  *lost   = NULL; // variável que vai receber uma imagem
ALLEGRO_BITMAP *won = NULL; // variável que vai receber uma imagem
ALLEGRO_TIMER *timer = NULL; //declarando o temporizador do jogo
ALLEGRO_FONT *font = NULL;
ALLEGRO_AUDIO_STREAM  *loserSong   = NULL; // variável que vai receber uma imagem
ALLEGRO_AUDIO_STREAM  *winnerSong  = NULL; // variável que vai receber uma imagem
ALLEGRO_BITMAP *avatar_d = NULL; // variável que vai receber um personagem 
ALLEGRO_BITMAP *avatar_e = NULL; // variável que vai receber um personagem 
ALLEGRO_BITMAP *avatar_c = NULL; // variável que vai receber um personagem 
ALLEGRO_BITMAP *avatar_b = NULL; // variável que vai receber um personagem 
...
ALLEGRO_AUDIO_STREAM *songSugar = NULL; //variável que vai receber uma das músicas do jogo
ALLEGRO_AUDIO_STREAM *opening = NULL; //variável que vai receber uma das músicas do jogo
ALLEGRO_BITMAP *frutinha = NULL; //variável que vai receber o bitmap das frutinhas
ALLEGRO_BITMAP *bonus = NULL; //variável que vai receber um o bitmap dos doces que dão o bônus
...
int move_cont = 0; // contador para controlar se a imagem do pacman está com a boca fechada ou aberta
bool mov_img[] = {false, false, false, false}; //vetor que definirá a direção da imagem do pacman
bool redraw = true; //variável para redesenhar a tela
bool killer = false; //variável para testar se o pacman morreu
bool is_open = true; 
...
bool intencao[4] = {false, false, false, false}; //Vetor que vai guardar com true o próximo movimento do personagem
...
unsigned char gposy_1 = 105; //posição do fantasma
unsigned char gposx_1 = 90; //posição do fantasma
...
//matriz definindo mapa do jogo: 1 representa paredes, 0 representa corredor, 2 representa bônus
char MAPA[17][17] ={
   "1111111111111111",
   "1000000110002001",
   "1011110000111101",
...
void set_mov(bool [], char); //prótotipo da função que atribui true à posição do movimento do pacman
void set_false(bool []); //prótotipo da função que atribui false à intenção de movimento caso ela já tenha sido atendida
...
int inicializa() {
    ...
    al_register_event_source(event_queue, al_get_timer_event_source(timer));//atribuindo os eventos de tempo à fila
    al_register_event_source(event_queue, al_get_keyboard_event_source()); //atribuindo os eventos do teclado à fila

    al_start_timer(timer); //inciando o timer

    //comaçando a música do jogo
    al_attach_audio_stream_to_mixer(opening, al_get_default_mixer());
    al_set_audio_stream_playing(opening, true);


    //Repetição que aguarda comando do usuário para começar o jogo
    while(1){
    ALLEGRO_EVENT ev0;
    al_wait_for_event(event_queue, &ev0);

    if(ev0.type == ALLEGRO_EVENT_KEY_DOWN){
        if(ev0.keyboard.keycode==ALLEGRO_KEY_SPACE){ //se o usuário apertar space para começar jogo
            al_draw_bitmap(map,0,0,0);                // Desenhando a imagem do mapa na tela
            al_draw_bitmap(avatar_d,eixo_x,eixo_y,0); // Desenhando o personagem no centro da tela
...
int main(int argc, char **argv){
  ...
  if(!inicializa()) return -1; //chamando a função que inicializa tudo que for necessário para o jogo

  srand (time(NULL)); 

  //trocando a música do jogo
  al_destroy_audio_stream(opening);
  ...
  al_set_audio_stream_playing(songSugar, true);
   ...
   while(1){
        ALLEGRO_EVENT ev;
        al_wait_for_event(event_queue, &ev);//armazendo o evento na fila de eventos
        ...
        if(ev.type == ALLEGRO_EVENT_TIMER){
          ...
          if(intencao[0] && MAPA[i-1][j] != '1'){//a intenção de movimento pode ser atendida no momento?
            mov_img[0] = true; //a posição do vetor correspondente à orientação do pacman recebe true
            mov_img[1] = false;
            mov_img[2] = false;
            mov_img[3] = false;

            intencao[0] = false; //a intenção, agora atendida, recebe false
            set_mov(teclas, 'U'); //atribui-se true à posição correspondente à "UP" do vetor que fazer o movimento (teclas)
            i--; // o personagem sobe uma linha na matriz
            eixo_y=i*c; // o personagem sobe uma linha na imagem do mapa
            move_cont++; //o contador muda o desenho do pacman de boca fechada para aberta ou vice-versa
         }
         //fazemos os mesmos testes para todas as situações de intenção de movimento
          ...
          //aqui começamos a testar se o movimento atual do pacman ainda pode proseguir
         else if(teclas[0] && MAPA[i-1][j] != '1'){// a próxima posição é um corredor?
            mov_img[0] = true; //a posição do vetor correspondente à orientação do pacman recebe true
            mov_img[1] = false;
            mov_img[2] = false;
            mov_img[3] = false;
         ...
         //fazemos os mesmos testes para cada direção de movimento
          ...
          redraw = true; //o mapa pode ser redesenhado
        ...
        else if(ev.type == ALLEGRO_EVENT_KEY_DOWN){ //se uma tecla for pressionada
            
            switch(ev.keyboard.keycode){ //vemos que tecla foi essa
                case ALLEGRO_KEY_UP:
                    if(MAPA[i-1][j] != '1'){ //se a posição não for uma parede
                        set_mov(teclas, 'U'); //o pacman pode se movimentar nessa direção e ela recebe true 
                        set_false(intencao); //a intenção recebe false
                    }
                    else
                        set_mov(intencao, 'U');// se o movimento não puder se feito agora, ele é armazenado como intenção

                    break;
                //fazendo os testes para as outras setinhas...
        ...
        if(redraw && al_is_event_queue_empty(event_queue)){
            ...
            al_draw_bitmap(map,0,0,0); //redesenhando a tela
            for (int i=0; i<17; i++)
                for(int j=0; j<16; j++) {
                    if(MAPA[i][j]=='0')
            ...
            if(move_cont%2 == 0){ //se o contador for par, vamos desenhar o pacman com a boca fechada
            ...
            else{ //se o contador for ímpar, vamos desenhar o pacman com a boca aberta
          ...
          al_flip_display(); //depois de redesenhar o pacman e os fantasmas na nova posição, trocamos a tela
    ...
    //mostrando a tela de vencedor ou perdedor depois do fim do jogo e colocando a música correspondente
    if(win){
    ...
    //aguardando uma interação do usuário para fechar a tela
    while(!win && is_open){
      al_draw_bitmap(lost,0,0,0); //desenhado a imagem na tela
      al_flip_display();
   ...
   //aguardando uma interação do usuário para fechar a tela
    while(win && is_open){
      al_draw_bitmap(won,0,0,0); //desenhado a imagem na tela
      al_flip_display();
...
void set_false(bool movs[]){ //função que atribui false a todas as posições do vetor
  movs[0] = false;
  movs[1] = false;
  movs[2] = false;
  movs[3] = false;  
...
void set_mov(bool movs[], char dir){ //função que atribui true à posição correspondente à direção da tecla apertada
    switch(dir){
        case 'U':
          movs[0] = true;
          movs[1] = false;
